  ...
  class Actor
    ...
    def create_content(file, file_name, dsid)
      ...
      save_characterize_and_record_committer do
        Sufia.config.after_create_content.call(generic_file, user) if Sufia.config.respond_to?(:after_create_content)
      end
    end
    ...
    def revert_content(revision_id, datastream_id)
      ...
      save_characterize_and_record_committer do
        Sufia.config.after_revert_content.call(generic_file, user, revision_id) if Sufia.config.respond_to?(:after_revert_content)
      end
    end
    ...
    def update_content(file, datastream_id)
      ...
      save_characterize_and_record_committer do
        Sufia.config.after_update_content.call(generic_file, user) if Sufia.config.respond_to?(:after_update_content)
      end
    end
    ...
    def update_metadata(attributes, visibility)
      ...
      save_and_record_committer do
        Sufia.config.after_update_metadata.call(generic_file, user) if Sufia.config.respond_to?(:after_update_metadata)
      end
    end
    ...
    def destroy
      ...
      Sufia.config.after_destroy.call(pid, user) if Sufia.config.respond_to?(:after_destroy)
    end
    ...
    def save
      Sufia::Utils.retry_rescue(3.times, RSolr::Error::Http) { generic_file.save }
      true
